...
deploy:
	...
	docker buildx build --platform linux/amd64,linux/arm64 --push -t cr.selcloud.ru/crypto-bundle/bc-wallet-ethereum-hdwallet:$(build_tag) .

	helm --kubeconfig ~/.kube/kubenet.config --kube-context k0s-dev-cluster upgrade --install bc-wallet-ethereum-hdwallet-api --set "global.build_tag=$(build_tag)" --set "global.env=$(env)"= ./deploy/helm/api --values=./deploy/helm/api/values.yaml --values=./deploy/helm/api/values_$(env).yaml
